function mf = calculate_mf_BaClO42(RH)
% calculate_mf_BaClO42
% Calculates mass fraction of Ba(ClO4)2 based on Relative Humidity.
% Range: 0.5609 < RH < 0.9954
% Data source: Paper 23 - The Osmotic and Activity Coefficients of Calcium, 
% Strontium and Barium Perchlorate at 25°

% Add util folder to path if robust_fzero is not found
if ~exist('robust_fzero', 'file')
    [filepath,~,~] = fileparts(mfilename('fullpath'));
    addpath(fullfile(filepath, '..', 'util'));
end

% Check RH bounds
if RH > 1
    error("RH should be 0 < RH < 1");
end
if RH < 0.5609 || RH > 0.9954
    error("RH %.4f is outside valid range [%.4f, %.4f] for Ba(ClO4)2", RH, 0.5609, 0.9954);
end

% Coefficients from polynomial fit: RH = A0 + A1*x + A2*x^2 + A3*x^3 + A4*x^4
A_4 = -1.431823735284023;
A_3 = 0.004410303301651;
A_2 = -0.226988200552912;
A_1 = -0.139541626336994;
A_0 = 1.000387606157413;

% Polynomial function
f = @(xi) RH - A_0 - A_1.*xi - A_2.*xi.^2 - A_3.*xi.^3 - A_4.*xi.^4;

% Solve for mass fraction using robust_fzero
mf = robust_fzero(f, 0.0325, 0.6490, 0.3408);

end
