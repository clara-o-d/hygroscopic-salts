...
salt_data = {
    % Endothermic salts
    {'NaCl', 58.443, 0.765, 0.99, 'calculate_mf_NaCl', 0};
    {'KCl', 74.551, 0.855, 0.99, 'calculate_mf_KCl', 0};
    {'NH4Cl', 53.491, 0.815, 0.99, 'calculate_mf_NH4Cl', 0};
    {'CsCl', 168.363, 0.82, 0.99, 'calculate_mf_CsCl', 0};
    {'NaNO3', 85.00, 0.971, 0.995, 'calculate_mf_NaNO3', 0};
    {'AgNO3', 169.87, 0.865, 0.985, 'calculate_mf_AgNO3', 0};
    {'KI', 165.998, 0.97, 0.995, 'calculate_mf_KI', 0};
    ...
    {'KClO3', 122.55, 0.981, 0.9926, 'calculate_mf_KClO3', 0}; % Adjusted Max (Limit 0.9936)
    {'NaBr', 102.89, 0.614, 0.9280, 'calculate_mf_NaBr', 0};   % Adjusted Max (Limit 0.9290)
    {'NaI', 149.89, 0.581, 0.9659, 'calculate_mf_NaI', 0};     % Adjusted Max (Limit 0.9669)
    {'KBr', 119.00, 0.833, 0.9518, 'calculate_mf_KBr', 0};     % Adjusted Max (Limit 0.9528)
    {'RbCl', 120.92, 0.743, 0.9517, 'calculate_mf_RbCl', 0};   % Adjusted Max (Limit 0.9527)
    {'CsBr', 212.81, 0.848, 0.9472, 'calculate_mf_CsBr', 0};   % Adjusted Max (Limit 0.9482)
    {'CsI', 259.81, 0.913, 0.9614, 'calculate_mf_CsI', 0};     % Adjusted Max (Limit 0.9624)
    
    % Exothermic salts (No source data provided to verify)
    {'LiCl', 42.4, 0.12, 0.97, 'calculate_mf_LiCl', 1};
    {'LiOH', 24, 0.85, 0.97, 'calculate_mf_LiOH', 0};
    {'NaOH', 40, 0.23, 0.97, 'calculate_mf_NaOH', 0};
    {'HCl', 36.5, 0.17, 0.97, 'calculate_mf_HCl', 0};
    {'CaCl2', 111, 0.31, 0.97, 'calculate_mf_CaCl', 1};
    {'MgCl2', 95.2, 0.33, 0.97, 'calculate_mf_MgCl', 0};
    {'MgNO3', 148.3, 0.55, 0.9, 'calculate_mf_MgNO3', 0};
    {'LiBr', 86.85, 0.07, 0.97, 'calculate_mf_LiBr', 0};
    {'ZnCl2', 136.3, 0.07, 0.97, 'calculate_mf_ZnCl', 0};
    {'ZnI2', 319.18, 0.25, 0.97, 'calculate_mf_ZnI', 0};
    {'ZnBr2', 225.2, 0.08, 0.85, 'calculate_mf_ZnBr', 0};
    {'LiI', 133.85, 0.18, 0.97, 'calculate_mf_LiI', 0};
    
    % Sulfates
    {'Na2SO4', 142.04, 0.9000, 0.9947, 'calculate_mf_Na2SO4', 0}; % Adjusted Boundaries
    {'K2SO4', 174.26, 0.9730, 0.9948, 'calculate_mf_K2SO4', 0};   % Adjusted Boundaries
    {'NH42SO4', 132.14, 0.8320, 0.9949, 'calculate_mf_NH42SO4', 0};% Adjusted Boundaries
    {'MgSO4', 120.37, 0.9060, 0.9950, 'calculate_mf_MgSO4', 0};   % Adjusted Boundaries
    {'MnSO4', 151.00, 0.9200, 0.9951, 'calculate_mf_MnSO4', 0};   % Adjusted Min (Limit 0.9190)
    {'Li2SO4', 109.94, 0.8540, 0.9946, 'calculate_mf_Li2SO4', 0}; % Adjusted Boundaries
    {'NiSO4', 154.75, 0.9720, 0.9952, 'calculate_mf_NiSO4', 0};   % Adjusted Min (Limit 0.9710)
    {'CuSO4', 159.61, 0.9760, 0.9953, 'calculate_mf_CuSO4', 0};   % Adjusted Boundaries
    {'ZnSO4', 161.44, 0.9390, 0.9952, 'calculate_mf_ZnSO4', 0};   % Adjusted Min (Limit 0.9380)
    
    % Nitrates (additional)
    {'BaNO3', 261.34, 0.9869, 0.9948, 'calculate_mf_BaNO32', 0}; % Adjusted Boundaries
    {'CaNO3', 164.09, 0.6474, 0.9945, 'calculate_mf_CaNO32', 0}; % Adjusted Boundaries
    
    % Halides (additional)
    {'CaBr2', 199.89, 0.6405, 0.9530, 'calculate_mf_CaBr2', 0}; % Adjusted Boundaries
    {'CaI2', 293.89, 0.8331, 0.9514, 'calculate_mf_CaI2', 0};   % Adjusted Boundaries
    {'SrCl2', 158.53, 0.8069, 0.9768, 'calculate_mf_SrCl2', 0}; % Adjusted Boundaries
    {'SrBr2', 247.43, 0.7786, 0.9561, 'calculate_mf_SrBr2', 0}; % Adjusted Boundaries
    {'SrI2', 341.43, 0.6795, 0.9559, 'calculate_mf_SrI2', 0};   % Adjusted Boundaries
    {'BaCl2', 208.23, 0.9385, 0.9721, 'calculate_mf_BaCl2', 0}; % Adjusted Boundaries
    {'BaBr2', 297.14, 0.8231, 0.9577, 'calculate_mf_BaBr2', 0}; % Adjusted Boundaries
    
    % Chlorates
    {'LiClO4', 106.39, 0.7785, 0.9869, 'calculate_mf_LiClO4', 0}; % Adjusted Min (Limit 0.7775)
};
