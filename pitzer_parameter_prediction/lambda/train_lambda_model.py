#!/usr/bin/env python3
"""
Train Machine Learning Models to Predict Lambda Pitzer Parameter at 25°C

Lambda parameters describe interactions between neutral species and ions.

This script demonstrates a complete ML workflow:
1. Load the processed lambda dataset
...
def load_data(filepath='lambda_processed.csv'):
    """Load the processed lambda dataset and separate features and targets."""
    print("=" * 80)
    print("Loading Lambda Parameter Data")
    print("=" * 80)
    
    df = pd.read_csv(filepath)
    print(f"✓ Loaded {len(df)} samples with {len(df.columns)} columns")
    
    # Define target column (lambda_a1 at 25°C)
    target_col = 'lambda_a1'
    ...
    exclude_cols = [
        target_col,
        'lambda_a2', 'lambda_a3', 'lambda_a4',  # Temperature-dependent parameters
    ]
    ...
    print(f"✓ Target: {target_col} (lambda parameter at 25°C)")
...
def plot_predictions(models, X_val_scaled, y_val, output_file='lambda_predictions.png', dataset_name="Validation"):
    ...
    fig.suptitle(f'Lambda Parameter Prediction: Predicted vs Actual ({dataset_name} Set)', 
    ...
    for i, (model_name, model) in enumerate(models.items()):
        ...
        ax.set_xlabel('Actual Lambda', fontsize=11)
        ax.set_ylabel('Predicted Lambda', fontsize=11)
...
def plot_best_model_test_predictions(best_model, best_model_name, X_test_scaled, y_test,
                                     output_file='best_lambda_model_test_predictions.png'):
    ...
    fig.suptitle('Lambda Parameter - Test Set Predictions: Best Model', 
                fontsize=16, fontweight='bold', y=0.995)
    ...
    ax.set_xlabel('Actual Lambda', fontsize=12)
    ax.set_ylabel('Predicted Lambda', fontsize=12)
...
def save_model(model, scaler, imputer, filepath='best_lambda_model.pkl'):
...
def main():
    ...
    print("\n" + "=" * 80)
    print("LAMBDA PITZER PARAMETER PREDICTION (25°C)")
    print("Neutral-Ion Interactions")
    print("=" * 80)
    
    # Set up paths
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, 'lambda_processed.csv')
    ...
    plot_output = os.path.join(script_dir, 'lambda_predictions.png')
    ...
    best_model_plot_output = os.path.join(script_dir, 'best_lambda_model_test_predictions.png')
    ...
    # 11. Save best model
    model_path = os.path.join(script_dir, 'best_lambda_model.pkl')
    ...
    print("=" * 80)
    print(f"\nDataset: Lambda Parameter (Neutral-Ion Interactions)")
    ...
    print("\nGenerated Files:")
    print("  - best_lambda_model.pkl (trained model + scaler)")
    print("  - lambda_predictions.png (all models on validation set)")
    print("  - best_lambda_model_test_predictions.png (best model on test set)")
