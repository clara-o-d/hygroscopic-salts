#!/usr/bin/env python3
"""
Train Machine Learning Models to Predict Psi Pitzer Parameter at 25°C

Psi parameters describe triple ion interactions (e.g., cation-anion-cation or cation-anion-anion).

This script demonstrates a complete ML workflow:
1. Load the processed psi dataset
...
def load_data(filepath='psi_processed.csv'):
    """Load the processed psi dataset and separate features and targets."""
    print("=" * 80)
    print("Loading Psi Parameter Data")
    print("=" * 80)
    
    df = pd.read_csv(filepath)
    print(f"✓ Loaded {len(df)} samples with {len(df.columns)} columns")
    
    # Define target column (psi_a1 at 25°C)
    target_col = 'psi_a1'
    
    # ID columns
    id_cols = ['species1', 'species2', 'species3', 'source']
    
    # Columns to exclude (target and other temperature-dependent parameters)
    exclude_cols = [
        target_col,
        'psi_a2', 'psi_a3', 'psi_a4',  # Temperature-dependent parameters
    ]
...
def plot_predictions(models, X_val_scaled, y_val, output_file='psi_predictions.png', dataset_name="Validation"):
    ...
    fig.suptitle(f'Psi Parameter Prediction: Predicted vs Actual ({dataset_name} Set)', 
    ...
    for i, (model_name, model) in enumerate(models.items()):
        ...
        ax.set_xlabel('Actual Psi', fontsize=11)
        ax.set_ylabel('Predicted Psi', fontsize=11)
...
def plot_best_model_test_predictions(best_model, best_model_name, X_test_scaled, y_test,
                                     output_file='best_psi_model_test_predictions.png'):
    ...
    fig.suptitle('Psi Parameter - Test Set Predictions: Best Model', 
                fontsize=16, fontweight='bold', y=0.995)
    ...
    ax.set_xlabel('Actual Psi', fontsize=12)
...
def save_model(model, scaler, imputer, filepath='best_psi_model.pkl'):
...
def main():
    ...
    print("\n" + "=" * 80)
    print("PSI PITZER PARAMETER PREDICTION (25°C)")
    print("Triple Ion Interactions")
    print("=" * 80)
    
    # Set up paths
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, 'psi_processed.csv')
    ...
    plot_output = os.path.join(script_dir, 'psi_predictions.png')
    ...
    best_model_plot_output = os.path.join(script_dir, 'best_psi_model_test_predictions.png')
    ...
    # 11. Save best model
    model_path = os.path.join(script_dir, 'best_psi_model.pkl')
    ...
    print("=" * 80)
    print(f"\nDataset: Psi Parameter (Triple Ion Interactions)")
    ...
    print("\nGenerated Files:")
    print("  - best_psi_model.pkl (trained model + scaler)")
    print("  - psi_predictions.png (all models on validation set)")
    print("  - best_psi_model_test_predictions.png (best model on test set)")
