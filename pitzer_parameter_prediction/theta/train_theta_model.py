#!/usr/bin/env python3
"""
Train Machine Learning Models to Predict Theta Pitzer Parameter at 25°C

Theta parameters describe interactions between ions of the same charge sign
(cation-cation or anion-anion interactions).

This script demonstrates a complete ML workflow:
1. Load the processed theta dataset
...
def load_data(filepath='theta_processed.csv'):
    """Load the processed theta dataset and separate features and targets."""
    print("=" * 80)
    print("Loading Theta Parameter Data")
    print("=" * 80)
    
    df = pd.read_csv(filepath)
    print(f"✓ Loaded {len(df)} samples with {len(df.columns)} columns")
    
    # Define target column (theta_a1 at 25°C)
    target_col = 'theta_a1'
    
    # ID columns
    id_cols = ['species1', 'species2', 'source']
    
    # Columns to exclude (target and other temperature-dependent parameters)
    exclude_cols = [
        target_col,
        'theta_a2', 'theta_a3', 'theta_a4',  # Temperature-dependent parameters
    ]
    ...
    return X, y, species_info, feature_cols, target_col
...
def split_and_scale_data(X, y, train_size=0.75, val_size=0.15, test_size=0.10, random_state=42):
...
def plot_predictions(models, X_val_scaled, y_val, output_file='theta_predictions.png', dataset_name="Validation"):
    ...
    fig.suptitle(f'Theta Parameter Prediction: Predicted vs Actual ({dataset_name} Set)', 
    ...
    for i, (model_name, model) in enumerate(models.items()):
        ...
        ax.set_xlabel('Actual Theta', fontsize=11)
        ax.set_ylabel('Predicted Theta', fontsize=11)
...
def plot_best_model_test_predictions(best_model, best_model_name, X_test_scaled, y_test,
                                     output_file='best_theta_model_test_predictions.png'):
    ...
    fig.suptitle('Theta Parameter - Test Set Predictions: Best Model', 
                fontsize=16, fontweight='bold', y=0.995)
    ...
    ax.set_xlabel('Actual Theta', fontsize=12)
    ax.set_ylabel('Predicted Theta', fontsize=12)
...
def save_model(model, scaler, imputer, filepath='best_theta_model.pkl'):
...
def main():
    ...
    print("\n" + "=" * 80)
    print("THETA PITZER PARAMETER PREDICTION (25°C)")
    print("Same-Charge Ion Interactions")
    print("=" * 80)
    
    # Set up paths
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, 'theta_processed.csv')
    
    # 1. Load data
    X, y, species_info, feature_cols, target_col = load_data(data_path)
    ...
    (X_train, X_val, X_test,
     X_train_scaled, X_val_scaled, X_test_scaled,
     y_train, y_val, y_test, scaler, imputer) = \
        split_and_scale_data(X, y, train_size=0.75, val_size=0.15, test_size=0.10, random_state=42)
    ...
    plot_output = os.path.join(script_dir, 'theta_predictions.png')
    ...
    best_model_plot_output = os.path.join(script_dir, 'best_theta_model_test_predictions.png')
    ...
    # 11. Save best model
    model_path = os.path.join(script_dir, 'best_theta_model.pkl')
    ...
    print("=" * 80)
    print(f"\nDataset: Theta Parameter (Same-Charge Ion Interactions)")
    print(f"  Samples: {len(X)} ({len(X_train)} train, {len(X_val)} validation, {len(X_test)} test)")
    print(f"  Features: {len(feature_cols)}")
    print(f"  Target: {target_col} at 25°C")
    ...
    print("\nGenerated Files:")
    print("  - best_theta_model.pkl (trained model + scaler)")
    print("  - theta_predictions.png (all models on validation set)")
    print("  - best_theta_model_test_predictions.png (best model on test set)")
