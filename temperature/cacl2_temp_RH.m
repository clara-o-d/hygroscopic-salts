...
% Script: cacl2_temp_RH.m
% 3D plot of CaCl2 mass-based water uptake as a function of RH and temperature
...
fprintf('Loading Pitzer parameters for CaCl2...\n');
csv_file = fullfile(filepath, '..', 'data', 'parsed_thermodb', 'pitzer_binary.csv');
pitzer_data = readtable(csv_file);

% Find CaCl2 parameters (Ca++ and Cl-)
cacl2_idx = find(strcmp(pitzer_data.species1, 'Ca++') & strcmp(pitzer_data.species2, 'Cl-'));
if isempty(cacl2_idx)
    % Try reverse order
    cacl2_idx = find(strcmp(pitzer_data.species1, 'Cl-') & strcmp(pitzer_data.species2, 'Ca++'));
end

if isempty(cacl2_idx)
    error('CaCl2 parameters not found in pitzer_binary.csv');
end

% Extract temperature-dependent coefficients
params.beta0_a1 = pitzer_data.beta0_a1(cacl2_idx);
params.beta0_a2 = pitzer_data.beta0_a2(cacl2_idx);
params.beta0_a3 = pitzer_data.beta0_a3(cacl2_idx);
params.beta0_a4 = pitzer_data.beta0_a4(cacl2_idx);

params.beta1_a1 = pitzer_data.beta1_a1(cacl2_idx);
params.beta1_a2 = pitzer_data.beta1_a2(cacl2_idx);
params.beta1_a3 = pitzer_data.beta1_a3(cacl2_idx);
params.beta1_a4 = pitzer_data.beta1_a4(cacl2_idx);

params.beta2_a1 = pitzer_data.beta2_a1(cacl2_idx);
params.beta2_a2 = pitzer_data.beta2_a2(cacl2_idx);
params.beta2_a3 = pitzer_data.beta2_a3(cacl2_idx);
params.beta2_a4 = pitzer_data.beta2_a4(cacl2_idx);

params.cphi_a1 = pitzer_data.cphi_a1(cacl2_idx);
params.cphi_a2 = pitzer_data.cphi_a2(cacl2_idx);
params.cphi_a3 = pitzer_data.cphi_a3(cacl2_idx);
params.cphi_a4 = pitzer_data.cphi_a4(cacl2_idx);

params.alpha1 = pitzer_data.alpha1(cacl2_idx);
params.alpha2 = pitzer_data.alpha2(cacl2_idx);

fprintf('CaCl2 Pitzer parameters loaded successfully.\n');
...
% RH range - CaCl2 is highly hygroscopic (DRH ~31%)
RH_vec = linspace(0.31, 0.95, 60);  % 31% to 95% RH (60 points)
...
nu = 3;              % Number of ions (Ca++ and 2 Cl-)
...
% Initialize arrays
water_uptake = zeros(length(T_C_vec), length(RH_vec));
...
for i = 1:length(T_C_vec)
    ...
    for j = 1:length(RH_vec)
        ...
        % CaCl2 is highly hygroscopic, similar to MgCl2
        if RH < 0.40
            m_guess = 8;   % Near deliquescence
        elseif RH < 0.60
            m_guess = 4;
        elseif RH < 0.80
            m_guess = 2;
        ...
        try
            % CaCl2 can reach very high concentrations
            m_solution = fzero(@(m) pitzer_residual(m, RH, beta0, beta1, beta2, cphi, ...
                params.alpha1, params.alpha2, nu, T_C), [0.01, 15], ...
        ...
        if ~isnan(m_solution) && m_solution > 0
            ...
            mass_salt_g = m_solution * MW_CaCl2;  % grams
...
if exist(atacama_rh_file, 'file') && exist(atacama_temp_file, 'file')
    ...
    if n_invalid > 0
        fprintf('  Warning: %d points below DRH or outside grid range\n', n_invalid);
    end
    ...
    atacama_data_available = (n_valid > 0);
else
...
zlabel('Water Uptake: kg H_2O / (kg H_2O + kg CaCl_2)', 'FontSize', 12, 'FontWeight', 'bold');
title('CaCl_2 Water Uptake vs RH and Temperature (Pitzer Model)', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
view(-30, 30);
set(gca, 'FontSize', 11);
print(fullfile(fig_out_dir, 'CaCl2_WaterUptake_3D'), '-dpng', '-r300');
...
xlabel('Relative Humidity (%)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Temperature (°C)', 'FontSize', 12, 'FontWeight', 'bold');
title('CaCl_2 Water Uptake: kg H_2O / (kg H_2O + kg CaCl_2) - Contour Plot', 'FontSize', 14, 'FontWeight', 'bold');
set(gca, 'FontSize', 11);
print(fullfile(fig_out_dir, 'CaCl2_WaterUptake_Contour'), '-dpng', '-r300');
...
title('CaCl_2: Water Mole Fraction vs RH and Temperature', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
view(-30, 30);
set(gca, 'FontSize', 11);
print(fullfile(fig_out_dir, 'CaCl2_MoleFraction_3D'), '-dpng', '-r300');
...
xlabel('Relative Humidity (%)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Temperature (°C)', 'FontSize', 12, 'FontWeight', 'bold');
zlabel('Molality (mol CaCl_2 / kg H_2O)', 'FontSize', 12, 'FontWeight', 'bold');
title('CaCl_2 Molality vs RH and Temperature', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
view(-30, 30);
set(gca, 'FontSize', 11);
print(fullfile(fig_out_dir, 'CaCl2_Molality_3D'), '-dpng', '-r300');

if atacama_data_available
    fprintf('All plots saved to: %s\n', fig_out_dir);
    if ~isempty(atacama_RH)
        fprintf('Atacama desert daily cycle overlaid on all plots (red line)\n');
    else
        fprintf('No Atacama points above CaCl2 deliquescence point\n');
    end
else
    fprintf('All plots saved to: %s\n', fig_out_dir);
end
