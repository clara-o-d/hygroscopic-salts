...
%% 1. Load Atacama Climate Data to Get RH Range
...
% Hygroscopic salts (low DRH - good for Atacama)
...
% Moderately hygroscopic (may work at high Atacama RH)
salt_props('Na+_Br-') = {102.89, 2, 0.614, 'NaBr'};
salt_props('Na+_I-') = {149.89, 2, 0.581, 'NaI'};

% Less hygroscopic (likely won't work)
...
for row = 1:height(pitzer_psi)
    ...
    elseif is_anion_1 && is_cation_2 && is_anion_3
        % X1-M-X2 format (two anions, one cation) - possible for multi-anion mixtures
    ...
    % Need to check the appropriate cation-anion pairs based on mixture type
    if ~isempty(cations)
...
fprintf('  Atacama-compatible (both salts DRH < %.1f%%): %d\n', atacama_max_RH*100, n_atacama_compatible);
...
if isempty(viable)
    ...
    fprintf('\nThis likely means no salt mixtures with complete Pitzer parameters\n');
    fprintf('(including both psi and theta) can deliquesce at Atacama RH conditions.\n');
else
    ...
    fprintf('      and can deliquesce at Atacama conditions. Multi-component Pitzer\n');
    fprintf('      calculations would be needed to evaluate actual water uptake swing.\n');
end

fprintf('\n%s\n', repmat('=', 1, 80));
fprintf('ATACAMA-COMPATIBLE MIXTURES (DRH check passed, but missing parameters)\n');
fprintf('%s\n', repmat('=', 1, 80));

% Show mixtures that pass DRH check but fail for other reasons
atacama_ok = rejected(arrayfun(@(x) ~contains(x.reason_rejected, 'exceed'), rejected));
if ~isempty(atacama_ok)
    fprintf('\n%-12s  %-12s  DRH1   DRH2   Issue\n', 'Salt 1', 'Salt 2');
    fprintf('%s\n', repmat('-', 1, 80));
    for i = 1:min(15, length(atacama_ok))
        fprintf('%-12s  %-12s  %5.1f%% %5.1f%% %s\n', ...
            atacama_ok(i).salt1, atacama_ok(i).salt2, ...
            atacama_ok(i).drh1*100, atacama_ok(i).drh2*100, ...
            atacama_ok(i).reason_rejected);
    end
    if length(atacama_ok) > 15
        fprintf('... and %d more\n', length(atacama_ok) - 15);
    end
else
    fprintf('None found.\n');
end
...
function has_params = check_salt_binary_params(pitzer_binary, cation, anion)
    ...
    % Check if beta0 parameter exists (at minimum)
    has_params = ~isnan(pitzer_binary.beta0_a1(idx));
end
...
function [has_theta, theta_value] = check_theta_params(pitzer_theta, ion1, ion2)
    ...
    % Get theta value
    theta_value = pitzer_theta.theta_a1(idx);
