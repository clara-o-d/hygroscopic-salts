...
%% --- PLOT: ln(Activity Coefficient) vs ln(RH) (NO LABELS) ---
...
for s = 1:num_salts
    ...
    % Plot ln(gamma) vs ln(RH) (NO text labels)
    plot(data.ln_RH, data.ln_gamma_w_ion, 'LineWidth', 2, ...
...
% Save figure
print(fullfile(fig_out_dir, 'ln_Activity_Coefficient_vs_ln_RH_No_Labels'), '-dtiff', '-r600');
fprintf('Saved: %s\n', fullfile(fig_out_dir, 'ln_Activity_Coefficient_vs_ln_RH_No_Labels.tif'));
